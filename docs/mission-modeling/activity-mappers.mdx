...
<RemoteCodeBlock
  ...
  url="https://raw.githubusercontent.com/PlanDev/plandev/develop/merlin-framework/src/main/java/gov/nasa/jpl/plandev/merlin/framework/ActivityMapper.java"
></RemoteCodeBlock>
...
describing the structure of the value being mapped (see [here](https://github.com/PlanDev/plandev/wiki/Value-Schemas) for more info)

The next two methods are inverses of each other: `deserializeValue()` and `serializeValue()`. It is the job of `deserializeValue()` to take a `SerializedValue` and map it, if possible, into the mapper's supported value. Meanwhile, `serializeValue()` takes an instance of the mapper's supported value and turns it into a [`SerializedValue`](https://github.com/PlanDev/plandev/wiki/Activity-Mappers#what-is-a-serializedvalue).

There are plenty of examples of value mappers over in the [contrib module](https://github.com/PlanDev/plandev/tree/develop/contrib/src/main/java/gov/nasa/jpl/plandev/contrib/serialization/mappers).
...
```java
package dev.plandev.foomissionmodel.activities;

import dev.plandev.foomissionmodel.Mission;
import dev.plandev.foomissionmodel.models.ImagerMode;
import dev.plandev.merlin.framework.annotations.ActivityType;
import org.apache.commons.math3.geometry.euclidean.threed.Vector3D;

import java.util.List;

import static dev.plandev.foomissionmodel.generated.ActivityActions.call;
import static dev.plandev.merlin.framework.ModelActions.*;
import static dev.plandev.merlin.framework.annotations.ActivityType.EffectModel;
import static dev.plandev.merlin.framework.annotations.Export.Parameter;
import static dev.plandev.merlin.framework.annotations.Export.Validation;
import static dev.plandev.merlin.protocol.types.Duration.SECOND;
...
```java
package dev.plandev.foomissionmodel.generated.activities;

import dev.plandev.contrib.serialization.mappers.NullableValueMapper;
import dev.plandev.contrib.serialization.rulesets.BasicValueMappers;
import dev.plandev.foomissionmodel.Mission;
import dev.plandev.foomissionmodel.activities.FooActivity;
import dev.plandev.foomissionmodel.mappers.FooValueMappers;
import dev.plandev.merlin.framework.ActivityMapper;
import dev.plandev.merlin.framework.ModelActions;
import dev.plandev.merlin.framework.ValueMapper;
import dev.plandev.merlin.protocol.driver.Topic;
import dev.plandev.merlin.protocol.model.InputType;
import dev.plandev.merlin.protocol.model.OutputType;
import dev.plandev.merlin.protocol.model.TaskFactory;
import dev.plandev.merlin.protocol.types.InstantiationException;
import dev.plandev.merlin.protocol.types.SerializedValue;
import dev.plandev.merlin.protocol.types.UnconstructableArgumentException;
import dev.plandev.merlin.protocol.types.Unit;
import dev.plandev.merlin.protocol.types.ValueSchema;
...
@Generated("dev.plandev.merlin.processor.MissionModelProcessor")
