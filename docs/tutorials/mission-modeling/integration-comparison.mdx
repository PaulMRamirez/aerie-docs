...
Now that we have explored multiple methods to implement integration in PlanDev, let's compare all of the methods in the PlanDev UI.
Compile the current version of the model (`./gradlew :missionmodel:build --refresh-dependencies`) and upload it into PlanDev.
