...
- `plandev_admin`, which can view all data and perform all actions
...
In order to interact with the PlanDev system, a role must first be provided access via the [Hasura Metadata](https://github.com/PlanDev/plandev/tree/develop/deployment/hasura/metadata).
...
| Permission | Meaning |
| -: | :- |
| `NO_CHECK` | The user may run the action/function with no restrictions. **The `plandev_admin` role will always be treated as having `NO_CHECK` permissions on all Actions and Functions.** |
...
| Permission | Meaning   |
|-:|:-|
| `NO_CHECK` | The user may run the action/function with no restrictions. **The `plandev_admin` role will always be treated as having `NO_CHECK` permissions on all Plan Merge Functions.** |
...
For more information, see the [Environment Variables document](https://github.com/PlanDev/plandev-gateway/blob/develop/docs/ENVIRONMENT.md) in the Gateway.
...
1. By default, users will use the `plandev_admin` role and have access to the `plandev_admin`, `user`, and `viewer` roles
