...
PlanDev includes the capability to model flexible temporal relations through the use of `Coexistence Goals`.
...
The following sections introduce the syntax to represent temporal relations in PlanDev and provide a number of examples.
...
Flexible Time Intervals are represented in PlanDev by means of `TimingConstraint.bounds`, which are composed of two `TimingConstraint.singletons` to model the lower and upper bound.
For example,
```ts
