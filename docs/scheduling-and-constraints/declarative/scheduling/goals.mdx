...
This document describes different scheduling goals available in PlanDev and how to author them using the TypeScript EDSL.
...
The PlanDev scheduler provides support for restricting when a goal applies with the `.applyWhen()` method in the `Goal` class.
...
This section describes how to create a new scheduling goal via the PlanDev UI. In the UI, open the scheduling pane by clicking on the top-right bar.

import schedulingPanel from './assets/scheduling-panel.png';

<figure>
  <img alt="PlanDev UI - Scheduling Panel" src={schedulingPanel} />
  <figcaption>Figure 1: PlanDev UI Scheduling Panel</figcaption>
</figure>
...
<figure>
  <img alt="PlanDev UI - Scheduling Goal Editor - New Goal" src={goalEditorNew} />
...
- `export default` signals to PlanDev that this is the function that defines the Goal.
...
<figure>
  <img alt="PlanDev UI - Invalid Recurrence Goal" src={invalidRecurrenceGoal} />
...
<figure>
  <img alt="PlanDev UI - Goal Context Menu" src={goalContextMenu} />
...
<figure>
  <img alt="PlanDev UI - Scheduling Goal Editor - Edit Goal" src={goalEditorEdit} />
