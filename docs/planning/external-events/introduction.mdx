...
These sources are defined using [JSON Schema](https://json-schema.org/) format with a [meta-schema](https://github.com/PlanDev/plandev-gateway/blob/develop/src/schemas/external-event-validation-schemata.ts) enforced to ensure certain fields such as `key` and `start_time` are present and in the correct format.
