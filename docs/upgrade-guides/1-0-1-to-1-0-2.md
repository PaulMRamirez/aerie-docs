...
The UI view `/view/validate` endpoint was moved from the gateway to the UI server. Any downstream code must now `POST` to `/view/validate` on the UI server. The UI server has the same API so no other changes are needed other than changing the target server. So if you are running PlanDev locally with the default ports and configuration you would change `http://localhost:9000/view/validate` to `http://localhost/view/validate`.
