...
There are a few changes to our `docker-compose.yml` file for the `plandev_action` and `plandev_workspace` services. If you maintain your own docker-compose file, you'll need to make these changes:

* Under the `plandev_action` container, in the `environment` section, add `HASURA_GRAPHQL_JWT_SECRET: "${HASURA_GRAPHQL_JWT_SECRET}"`
* Under the `plandev_action` container, in the `environment` section, you can remove the `HASURA_GRAPHQL_ADMIN_SECRET` variable as it is no longer needed
* Under the `plandev_workspace` container, add `HASURA_GRAPHQL_URL: http://hasura:8080/v1/graphql` (or your equivalent, if running on a different url/port)

See this file in the `plandev` repo for a full example: https://github.com/PlanDev/plandev/blob/develop/deployment/docker-compose.yml

### Hasura metadata yaml file changes

If you maintain your own versions of Hasura metadata files for granular control of permissions (rare), you may need to update them to contain the latest values for Workspaces. [This PR is a good reference](https://github.com/PlanDev/plandev/pull/1757/files#diff-def18dd27c7d2c9dfe61a27ba55973f94e8354160d817a99c7ef0ad4c0682e7a) demonstrating the changes to the `yaml` files in `deployment/hasura/metadata`
...
Users can either create these modules directly or compile them using `rollup` or similar. We recommend using our [sequence adaptation template repo](https://github.com/PlanDev/plandev-sequence-adaptation-template) as a starting point, which demonstrates a Typescript adaptation which is compiled with rollup to a CommonJS module.

## Actions API changes

A new version of the `plandev-actions` library, `1.1.0` has been released & published to NPM as a part of this release. These changes are *non-breaking*, but users writing custom actions should upgrade their `plandev-actions` dependency to take advantage of the latest features, which include:
