...
:::caution
**PlanDev v2.8.0** is an **Upgrade Checkpoint** version - if you are upgrading from an _earlier_ version of PlanDev directly to a _later_ one, you **must first** upgrade to `v2.8.0` and follow the instructions below before continuing to a later version. See the [Upgrade Process](/deployment/upgrade-process/) guide for more details
:::

This document describes the upgrade instructions from `2.7.1` to `2.8.0`.

PlanDev v2.8.0 contains significant changes to our PostgreSQL database structure, along with changes to our required environment variables and configuration. It therefore requires a different upgrade process than usual, especially if you plan to migrate your data forward to v2.8.0. Make sure to read all instructions below carefully before continuing, and please open a Github issue or ask for help in our Slack room if you have any questions.


## For PlanDev End-Users
(ie. those running PlanDev from the released `Deployment.zip` or the `deployment` folder in the repository)
...
- existing values, especially PLANDEV_USERNAME, PLANDEV_PASSWORD, POSTGRES_USER and POSTGRES_PASSWORD, **must** remain unchanged from your previous version if you plan to migrate your data
...
* If you plan to migrate your data, follow the [migration instructions below](#for-anyone-migrating-data-to-v280) before running PlanDev v2.8.0 for the first time.

## For PlanDev Developers
(ie. anyone running PlanDev from a cloned version of the `plandev` repository)
...
* If you have ever changed the default names for the PlanDev databases (`plandev_merlin`, `plandev_scheduler`, `plandev_sequencing`, `plandev_ui`) - this is no longer allowed, and you **must** rename them to their default names.

### Migration Process
1. Starting from the deployment directory, run `cd merge_plandev_db`
...
4. As a precaution, your **old** databases have been preserved at this point. If the last step failed for any reason, you can safely drop the `plandev` database, troubleshoot the problem and re-run the `merge_db.sh` script
5. After the migration has run successfully, run `drop_old_dbs.sh` to drop your old databases.

Your data should now be fully migrated and ready to run PlanDev v2.8.0! Note that, due to breaking changes to the mission model framework in v2.7.0, you may need to **recompile and re-upload** your mission models if you are upgrading from a version **before v2.7.0**.
...
- plandev_gateway:
  - removed envvars:
    - `POSTGRES_PLANDEV_MERLIN_DB`
    - `POSTGRES_USER`
    - `POSTGRES_PASSWORD`
  - renamed envvars:
    - `POSTGRES_HOST` -> `PLANDEV_DB_HOST`
    - `POSTGRES_PORT` -> `PLANDEV_DB_PORT`
  - added envvars:
...
- plandev_merlin:
  ...
  - renamed envvars:
    - `MERLIN_DB_SERVER` -> `PLANDEV_DB_HOST`
    - `MERLIN_DB_PORT` -> `PLANDEV_DB_PORT`
...
- plandev_merlin_workers: (for each worker)
  - removed envvars:
    - `MERLIN_WORKER_DB`
    - `MERLIN_WORKER_DB_USER`
    - `MERLIN_WORKER_DB_PASSWORD`
  - renamed envvars:
    - `MERLIN_WORKER_DB_SERVER` -> `PLANDEV_DB_HOST`
    - `MERLIN_WORKER_DB_PORT` -> `PLANDEV_DB_PORT`
  - added envvars:
...
- plandev_scheduler:
  ...
  - renamed envvars:
    - `SCHEDULER_DB_SERVER` -> `PLANDEV_DB_SERVER`
    - `SCHEDULER_DB_PORT` -> `PLANDEV_DB_PORT`
...
- plandev_scheduler_workers: (for each worker)
  - removed envvars:
    - `SCHEDULER_WORKER_DB`
    - `SCHEDULER_WORKER_DB_USER`
    - `SCHEDULER_WORKER_DB_PASSWORD`
  - renamed envvars:
    - `SCHEDULER_WORKER_DB_SERVER` -> `PLANDEV_DB_SERVER`
    - `SCHEDULER_WORKER_DB_PORT` -> `PLANDEV_DB_PORT`
  - added envvars:
...
- plandev_sequencing:
  ...
  - renamed envvars:
    - `SEQUENCING_DB_SERVER` -> `PLANDEV_DB_HOST`
    - `SEQUENCING_DB_PORT` -> `PLANDEV_DB_PORT`
...
- hasura:
  - removed envvars:
    - `PLANDEV_MERLIN_DATABASE_URL`
    - `PLANDEV_SCHEDULER_DATABASE_URL`
    - `PLANDEV_SEQUENCING_DATABASE_URL`
    - `PLANDEV_UI_DATABASE_URL`
  - added envvars:
    - `PLANDEV_DATABASE_URL: "postgres://${PLANDEV_USERNAME}:${PLANDEV_PASSWORD}@postgres:5432/plandev?options=-c%20search_path%3Dutil_functions%2Chasura%2Cpermissions%2Ctags%2Cmerlin%2Cscheduler%2Csequencing%2Cpublic"`
- postgres:
  - added envvars:
    - `GATEWAY_DB_USER: "${GATEWAY_USERNAME}"`
    - `GATEWAY_DB_PASSWORD: "${GATEWAY_PASSWORD}"`
